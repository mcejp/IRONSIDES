...
procedure printTooManyRecordTypesInNSECRecordWarning(LineCount : in Unsigned_Types.Unsigned32) is
begin
   Ada.Text_Io.Put("Warning:  Maximum supported number of record types exceeded ");
   Ada.Text_IO.New_Line;
   Ada.Text_IO.put("in NSEC record at line # ");
   m_io.put(item => lineCount, width => 1);
   Ada.Text_IO.New_Line;
   Ada.Text_Io.Put("Maximum number is ");
   Ada.Text_IO.put(item => integer'image(rr_type.nsec_record_type.maxNumberOfRecordTypes));
   Ada.Text_Io.Put(".  Further record types ignored.");
   Ada.Text_IO.New_Line;
   Ada.Text_IO.Put("To process the entire NSEC record, source code must be recompiled ");
   Ada.Text_IO.New_Line;
   Ada.Text_IO.Put("with larger value of rr_type.nsec_record_type.maxNumberOfRecordTypes and revalidated.");
   Ada.Text_Io.New_Line;
...
procedure printMissingRecordTypeErrorInfo(currentLine : in rr_type.LineFromFileType;
      Length : in Natural; LineCount : in Unsigned_Types.Unsigned32) is
begin
